...
export default function Onboarding() {
  ...
  const handleGenerate = async () => {
    ...
    addLog('Onboarding data saved locally', 'success');
    ...
    addLog(`Topic resolved: "${topic}"`, 'info');
    addLog(`Skill level: ${skillLevel}`, 'info');
    addLog('Sending POST /api/roadmaps/generate/ ...', 'info');

    try {
      const startTime = Date.now();
      const roadmap = await generateRoadmap(topic, skillLevel);
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      addLog(`SUCCESS! Roadmap generated in ${elapsed}s`, 'success');
      addLog(`Course: ${roadmap?.course?.title || roadmap?.title || 'Unknown'}`, 'success');
      navigate('/roadmap');
    } catch (error) {
  ...
  return (<PublicLayout showFooter={false}>
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center py-12 px-4">
      <Card className="w-full max-w-2xl">
        ...
        <CardContent>
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
              {step === 1 && (<div className="grid gap-3">
                {skillLevels.map((level) => (<div key={level.value} onClick={() => setSkillLevel(level.value)} className={cn('p-4 rounded-lg border-2 cursor-pointer transition-all', skillLevel === level.value ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50')}>
              ...
              {step === 2 && (<div className="space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {availableLanguages.map((lang) => (<Badge key={lang} variant={selectedLanguages.includes(lang) ? 'default' : 'outline'} className="cursor-pointer" onClick={() => toggleItem(lang, selectedLanguages, setSelectedLanguages)}>
                      {lang}
                    </Badge>))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {availableTools.map((tool) => (<Badge key={tool} variant={selectedTools.includes(tool) ? 'default' : 'outline'} className="cursor-pointer" onClick={() => toggleItem(tool, selectedTools, setSelectedTools)}>
                      {tool}
                    </Badge>))}
                  </div>
                </div>
              ...
              {step === 3 && (<div className="space-y-6">
                <div className="grid gap-3">
                  {learningGoals.slice(0, 3).map((goal) => (<div key={goal.id} onClick={() => setSelectedGoal(goal.id)} className={cn('p-4 rounded-lg border-2 cursor-pointer transition-all', selectedGoal === goal.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50')}>
          ...
          <div className="flex justify-between mt-8">
            ...
            {step < totalSteps ? (<Button onClick={() => setStep(s => s + 1)}>
              Next <ArrowRight className="ml-2 h-4 w-4" />
            </Button>) : (<Button onClick={handleGenerate} disabled={!selectedGoal || isGenerating}>
      ...
      {/* Debug Log Panel */}
      {showLogs && (
        <Card className="w-full max-w-2xl mt-4 bg-black/90 text-green-400 font-mono text-xs">
          <CardHeader className="py-2 px-4 flex flex-row items-center justify-between border-b border-green-900">
            <div className="flex items-center gap-2">
              <Terminal className="h-4 w-4" />
              <span>API Debug Log</span>
            </div>
            <Button variant="ghost" size="sm" onClick={() => setShowLogs(false)} className="h-6 text-green-400 hover:text-green-300">
              Hide
            </Button>
          </CardHeader>
          <CardContent className="p-4 max-h-48 overflow-y-auto">
            {logs.length === 0 ? (
              <p className="text-green-600">Click "Generate My Roadmap" to see logs...</p>
            ) : (
              logs.map((log, i) => (
      ...
      {!showLogs && (
        <Button variant="outline" size="sm" onClick={() => setShowLogs(true)} className="mt-4">
          <Terminal className="mr-2 h-4 w-4" /> Show Debug Logs
        </Button>
      )}
    </div>
  </PublicLayout>);
